...
test('creates and reads track metadata', async ({ request }) => {
  ...
  await expect(getTrackResponse).toBeOK();
...
test('returns a presigned upload target without requiring a real S3 bucket', async ({
  request,
}) => {
